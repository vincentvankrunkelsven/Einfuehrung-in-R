...
## Wie R funktioniert
Auf der rechten Seite geben Sie im Editor den R Code ein, um die Übung zu lösen. Wenn Sie auf den Button "Antwort abschicken" klicken, wird jede Codezeile interpretiert und durch R ausgeführt und Sie erhalten eine Nachricht, ob Ihr Code richtig war. Ihre Ausgabe des R-Codes wird in der Konsole unten rechts in der Ecke des Bildschirmes angezeigt.

R verwendet für die Kommentare die #-Taste, sodass Sie und andere Nutzer wissen, was der R Code bedeutet. Wie bei Twitter! Kommentare werden nicht als R Code ausgeführt, somit beeinflussen sie die Ergebnisse nicht. Zum Beispiel _Berechnen Sie 3+4_ ist im Editor rechts ein Kommentar.

Sie können die R Befehle auch direkt in der Konsole ausführen. Dies ist ein guter Weg, um mit den R Codes zu experimentieren, da Ihre Eingabe nicht auf die Richtigkeit überprüft wird.
...
*** =hint
Fügen Sie einfach eine Linie von R Code ein, der die Summe von 6 und 12 berechnet, genau wie das Beispiel in dem Beispielcode.
...
*** =sct
```{r}
test_output_contains("18", incorrect_msg = "Stellen Sie sicher, dass Sie '6 + 12' in einer neuen Zeile hinzugefügt haben. Starten Sie die Linie nicht mit einer '#'-Taste, ansonsten wird Ihr Code nicht ausgeführt!")
success_msg("Ausgezeichnet! Sehen Sie, wie die Konsole das Ergebnis des R Codes zeigt. Nun, da Sie mit der Schnittstelle vertraut sind, wollen wir uns an die Arbeit mit R machen!")
```
...
Eine Variable ermöglicht es Ihnen einen Wert (z.B. 4) oder ein Objekt (z.B. Funktionsbeschreibung) in R zu speichern. Später können Sie den Namen der Variable nutzen, um einfach auf den Wert oder das Objekt zuzugreifen, der innerhalb dieser Variable hinterlegt ist.  
...
Angenommen, Sie haben einen Obstkorb mit fünf Äpfeln. Als Datenanalyst in der Ausbildung, möchten Sie die Anzahl an Äpfeln in einer Variable mit den Namen `my_apples` speichern. 
...
*** =hint
Erinnern Sie sich daran, dass wenn Sie in R einen Wert oder ein Objekt einer Variable zuweisen, können Sie den Zuweisungsoperator `<-` verwenden. Alternativ können Sie `=` benutzen, aber der Operator `<-` ist in der R Community weit verbreitet.
...
success_msg("Großartig! Machen Sie mit der nächsten Übung weiter!")
```
...
Im Allgemeinen ist es bekannt, dass man Äpfel und Orangen nicht zusammenaddieren kann. Aber hey, das ist das, was Sie gemacht haben, nicht wahr :-)? Die Variablen `my_apples` und `my_oranges` haben eine Zuordnung in der vorherigen Übung erhalten. In R arbeitet der Operator `+` mit numerischen Variablen. Wenn Sie für die Variable `my_oranges` (siehe im Editor) einen Textwert zugeordnet haben und dann versuchen "apples" und "oranges" zu addieren, werden Sie schell feststellen, dass dies nicht möglich ist. Sie können nicht numerische Variablen und Zeichenvariablen addieren und der Variable `my_fruit` zuordnen. 
...
- Passen Sie den Code so an, dass R weiß, dass Sie 6 Orangen und einen Obstkorb mit 11 Stücke vom Obst haben.
...
success_msg("Wunderbar, Sie leisten gute Arbeit. Fahren die mit der nächsten Übung fort.")
```
...
Beachten Sie, wie die Anführungszeichen auf der rechten Seite zeigt, dass "einige Texte" ein Zeichen sind.
...
Können Sie sich daran erinnern, dass wenn Sie `5 + "six"` addieren, Sie eine Fehlermeldung aufgrund fehlender Übereinstimmung in Datentypen bekamen? Vermeiden Sie die Situation vorher, indem Sie die Variable prüfen. Dafür können Sie die Funktion nutzen, wie Sie es auf der rechten Seite sehen können.
