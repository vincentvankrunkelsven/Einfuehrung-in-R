...
title       : Matrizen
description : "In diesem Kapitel werden Sie lernen, wie Sie in R mit Matrizen arbeiten. Am Ende dieses Kapitels werden Sie in der Lage sein Matrizen zu erstellen und verstehen, wie Sie grundlegende Berechnungen mit ihnen machen können.
Sie werden Matrizen verwenden um die Kassenzahlen von Star Wars analysieren.
...
- Das erste Argument ist die Sammlung von Elementen, die R in die Zeilen und Spalten einer Matrix ordnet. Hier verwenden wir `1:9`, die eine Abkürzung für `c(1, 2, 3, 4, 5, 6, 7, 8, 9)` ist.
- Das Argument `byrow` zeigt an, dass die Matrix zeilenweise gefüllt wird. Wenn wir die Matrix spaltenweise füllen wollen, dann verwenden wir einfach`byrow = FALSE`. 
- Das dritte Argument `nrow` zeigt an, dass die Matrix drei Zeilen haben sollte.

*** =instructions
Erstellen Sie eine Matrix mit 3 Zeilen mit den Zahlen 1 bis 9, gefüllt zeilenweise. 

*** =hint
Lesen Sie die Aufgabe sorgfältig, denn die Antwort ist darin gegeben!
...
test_function("matrix", c("data", "byrow", "nrow"),
              incorrect_msg = "Haben Sie eine richtige Matrix erstellt? Werfen Sie einen Blick auf die Aufgabe, die Antwort ist dort gegeben!")
test_output_contains("matrix(1:9, byrow=TRUE, nrow=3)",
                     incorrect_msg = "Es scheint ein Problem in der Matrix Definition zu geben. Werfen Sie einen Blick auf die Aufgabe, die Antwort ist dort gegeben!")
...
Es ist Zeit Ihre Hände schmutzig zu machen. In der folgenden Übung werden Sie die Kassenzahlen der Star Wars Franchise analysieren. Möge die Macht mit Ihnen sein!

Im Editor sind drei Vektoren definiert. Jeder stellt die Kassenzahlen der ersten drei Filme dar. Das erste Element jedes Vektors zeigt die USA-Kassenzahlen, das zweite Element bezieht sich auf die Nicht-USA-Kassenzahlen (Quelle: Wikipedia).

In dieser Übung werden Sie all diese Zahlen in einem einzigen Vektor kombinieren. Als Nächstes werden Sie eine Matrix aus diesen Vektoren bilden.

*** =instructions
- Verwenden Sie `c(new_hope, empire_strikes, return_jedi)`, um die drei Vektoren in einen Vektor zu verbinden. Nennen Sie diesen Vektor `box_office`.
...
Damit Sie sich daran erinnern, was in `star_wars_matrix` gespeichert ist, möchten Sie die Filmnamen in die Zeilen hinzufügen. Nicht nur, dass diese Ihnen helfen werden, die Daten zu lesen, sie sind auch sehr nützlich, bestimmte Elemente aus der Matrix zu wählen.

Ähnlich wie bei den Vektoren, können Sie die Namen für die Zeilen und Spalten einer Matrix hinzufügen.
...
Das wichtigste für einen Film, um eine Legende in Hollywood zu werden, ist ihre weltweite Kassenzahl.

Um die Gesamteinnahmen der Kassen für die drei Star Wars Filme zu berechnen, müssen Sie die Summe von den Spalten US-Einnahmen und Non-US-Einnahmen nehmen.
...
Ihr R Arbeitsbereich, in dem alle Variablen 'live' definiert sind ([prüfen Sie, was ein Arbeitsbereich ist](http://www.statmethods.net/interface/workspace.html)), wurden bereits initialisiert und enthält zwei Matrizen:

- `star_wars_matrix` , die wir alle zusammen mit den Daten der ersten Triologie verwendet haben
- `star_wars_matrix2`, mit ähnlichen Daten für die zweite Triologie

Geben Sie den Namen dieser Matrizen in der Konsole ein und drücken Sie auf Enter, wenn Sie einen genaueren Blick haben wollen. Wenn Sie den Inhalt des Arbeitsbereichs prüfen möchten, können Sie `ls()` in der Konsole eingeben.
...
Genau wie jede [`cbind()`](http://www.rdocumentation.org/packages/base/functions/cbind) hat sie [`rbind()`](http://www.rdocumentation.org/packages/base/functions/cbind), jede [`colSums()`](http://www.rdocumentation.org/packages/base/functions/colSums) hat sie eine [`rowSums()`](http://www.rdocumentation.org/packages/base/functions/colSums). Ihr R Arbeitsbereich enthält bereits die `all_wars_matrix`, die Sie in der vorherigen Übung konstruiert hatten; geben Sie `all_wars_matrix` ein, um einen anderen Blick zu haben. Lassen Sie uns die Gesamteinnahmen der Kinokassen für die gesamte Saga berechnen.
...
Wenn Sie alle Elemente einer Zeile oder einer Spalte auswählen möchten, wird keine Zahl vor oder nach dem Komme benötigt, bzw.:
...
Ähnlich wie Sie es bei den Vektoren gelernt haben, gibt es auch bei den Matrizen in R Standardoperatoren, wie `+`, `-`, `/`, `*`, usw., die in einer elementweise Art in Matrizen arbeiten.

Zum Beispeil: `2 * my_matrix` multipliziert jedes Element von `my_matrix` mal zwei.

Als neu eingestellter Datenanalyst für Lucasfilm ist Ihre Aufgabe herauszufinden, wie viele Besucher in jedem Film für jeden geografischen Bereich gingen. Sie haben schon die Gesamteinnahmen in `all_wars_matrix`. Angenommen der Preis pro Ticket lag bei 5 Dollar. Teilen Sie einfach die Kasseneinnahmen durch die Ticketpreise, dann ergibt sich die Anzahl der Besucher.

*** =instructions
- Teilen Sie `all_wars_matrix` durch 5, geben Sie die Anzahl der Besucher in Millionen. Weisen Sie die resultierende Matrix `visitors` zu.
...
*** =hint
- Sie können die Funktion `mean()` verwenden, um den Mittelwert der Eingänge an die Funktion zu berechnen.
