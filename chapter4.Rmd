...
In Anbetracht der Bedeutung dieser Faktoren bei der Datenanalyse, sollten Sie anfangen zu lernen, wie sie erstellt, unterteilt und verglichen werden.
...
Es ist wichtig, dass R weiß, ob sie mit einer fortlaufenden oder kategorialen Variable zu tun hat, wie die statistischen Modelle, die Sie zukünftig bei beiden Typen unterschiedlich entwickeln werden. (Sie sehen später, warum dies der Fall ist.)

Ein gutes Beispiel für kategoriale Variablen ist die Variable 'Geschlecht'. Ein menschliches Individuum kann entweder "männlich" oder "weiblich" sein. So sind hier "männlich" und "weiblich", im vereinfachten Sinn, die beiden Werte der kategorialen Variable "Geschlecht" und jede Beobachtung kann entweder den Wert "männlich" oder "weiblich" zugewiesen werden.
...
*** =sct
```{r}
test_object("theory", incorrect_msg = "Stellen Sie sicher, dass Sie die Zeichenfolge `\"factors for categorical variables\"` der `theory` zugewiesen haben. Denken Sie in R an die Groß- und Kleinschreibung.");
...
Es ist klar, dass es zwei Kategorien gibt oder in R-Sprache **'Faktor Levels'**, bei der Aufgabe hier: "männlich" und "weiblich".
...
*** =hint
Klicken Sie einfach auf den Knopf 'Antwort abschicken' und werfen Sie einen Blick auf die Konsole. Beachten Sie, wie R die Reihenfolge der Faktor Levels für die ordinalskalierte Variablen anzeigt.
...
*** =sct
```{r}
msg <- "Ändern Sie nicht das Beispielcode. Drücken Sie einfach den Antwort abschicken - Button und inspizieren Sie die Lösung!"
...
Wenn Sie zuerst einen Datensatz bekommen, werden Sie oft bemerken, dass die Faktoren bestimmte Faktor Levels enthalten. Manchmal wollen Sie die Namen dieser Levels für Klarheit oder aus anderen Gründen ändern. R erlaubt Ihnen dies mit der Funktion [`levels()`](http://www.rdocumentation.org/packages/base/functions/levels) zu tun:

```
levels(factor_vector) <- c("name1", "name2",...)
```

Eine gute Illustration sind die Rohdaten, die durch eine Umfrage zur Verfügung gestellt wurden. Eine Standardfrage für jeden Fragebogen ist das Geschlecht der Befragten. Sie erinnern sich daran, dass in der vorherigen Frage dies ein Faktor ist und wenn Sie den Fragebogen auf der Straße ausführen, werden die Levels oftmals mit `"M"` und `"F"` verschlüsselt.

```
survey_vector <- c("M", "F", "F", "M", "M")
```

Als nächstes, wenn Sie Ihre Datenanalyse starten wollen, ist Ihr Hauptanliegen einen schönen Überblick über alle Variablen zu haben und was sie bedeuten. An diesem Punkt werden Sie oft die Faktor Levels zu `"Male"` und `"Female"` ändern wollen, statt `"M"` und `"F"`, um Ihr Leben leichter zu machen.
...
*** =hint
Achten Sie auf die Reihenfolge, in der Sie die Faktor Levels eingeben müssen. Hinweis: Blick auf die Reihenfolge in der die Werte ausgegeben werden sollen, wenn Sie `levels(factor_survey_vector)` eingeben.
...
Nach dem Beenden dieses Kurses wird dies einer Ihrer Lieblingsfunktionen in R sein [`summary()`](http://www.rdocumentation.org/packages/base/functions/summary). Diese Funktion gibt Ihnen einen schnellen Überblick über den Inhalt einer Variable:
...
*** =hint
Rufen Sie die Funktion [`summary()`](http://www.rdocumentation.org/packages/base/functions/summary)für beide Vektoren `survey_vector` und `factor_survey_vector` auf, so einfach ist das!
...
*** =hint
Klicken Sie einfach auf 'Antwort abschicken' und werfen Sie einen Blick auf die Ausgabe, die in der Konsole ausgegeben wird.
...
success_msg("Puh, es scheint so, dass R geschlechtsneutral ist. Oder vielleicht will R sich gerade aus diesen Diskussionen raushalten ;-).")
```
...
Lassen Sie uns sagen, dass Sie ein Forschungsteam mit fünf Datenanalysten leiten und dass Sie ihre Leistung bewerten wollen. Um das zu tun, werden Sie ihre Geschwindigkeit verfolgen, jeden Analysten als `"langsam"`, `"schnell"` oder `"verrückt"` bewerten und die Ergebnisse in `speed_vector` speichern.
...
*** =sct
```{r}
msg = "Ändern Sie nichts am Befehl, der die `speed_vector` Variablen gibt."
...
- Prüfen Sie, ob `da2` größer als `da5` ist; geben Sie einfach das Ergebnis aus. Denken Sie daran, dass Sie den Operator `>` verwenden können. So können Sie prüfen, ob ein Element größer als das andere ist.
...
success_msg("Bellissimo! Was bedeutet das Ergebnis? Der Datenanalyst zwei beschwert sich über den Datenanylsten fünf, während er oder sie in der Tat der/die Eine ist, der/die alles verlangsamt! Diese letzte Übung schließt das Kapitel über die Faktoren. Mit einer soliden Basis in Vektoren, Matrizen und Faktoren, sind Sie bereit, in die wunderbare Welt der Datenframes einzutauchen, die eine sehr wichtige Datenstruktur in R ist!")
```
